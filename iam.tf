...
resource "aws_iam_role_policy_attachment" "attash-paramter-store" {
    role       = aws_iam_role.parameter-store-role.name
...
resource "aws_iam_policy" "policy-ecs" {
  name        = "policy-ecs"
  path        = "/"
  description = "POlicy for ssm and cw"
...
resource "aws_iam_role" "parameter-store-role" {
